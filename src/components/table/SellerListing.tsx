...
import {
  ...
  UseAdminSellerActions,
  UseExportAdminSellerList,
  UseGetAdminBuyerList,
  UseGetAdminSellerList,
...
export default function BuyerListing() {
  ...
  const [succOpen1, setSuccOpen1] = useState(false);

  const closeModel = (val: any) => {
    setSuccOpen(val);
  };
  const closeModel1 = (val: any) => {
    setSuccOpen1(val);
  };
  ...
  const successCallback = (data: any) => {
    if (data.onboardingInquiriesResponses)
      setSellerList(data.onboardingInquiriesResponses);
    setTotalRecords(data.totalRecords);
  };
  const failureCallback = () => {};
  const { mutate: getAdminSellerList, isPending } = UseGetAdminSellerList(
    successCallback,
    failureCallback,
  );
  ...
  }, [getAdminSellerList, page, itemsPerPage, searchText, debouncedResults]);

  // Export Buyer List
  const successCallbackForExport = (data: any) => {
    if (data) {
      const blob = new Blob([data], { type: "text/csv" });
      saveAs(blob, `seller-list-${new Date().getTime()}.csv`);
    }
  };
  const failureCallbackForExport = () => {};
  const { mutate: exportAdminSellerList, isPending: exportPending } =
    UseExportAdminSellerList(
  ...
  return (
    <div className="w-full">
      <div className="mb-6">
        ...
        <div className="flex items-center justify-between gap-4 mb-4">
          <div className="relative flex-1">
            <Input
              placeholder="Search by Seller name, Company Name, Company registration number"
          ...
          {/* <ButtonComponent
            variant="outline"
            className="text-sm leading-normal font-semibold px-8 py-[9.5px]"
          >
            Export
          </ButtonComponent> */}
        </div>
            ...
            {isPending ? (
              ...
              sellerList?.map((seller: any) => (
                <TableRow key={seller.id}>
                  ...
                  <TableCell className="border-b border-r pl-[54px] pe-[41px] py-[22px] text-nowrap text-sm leading-4 break-words max-w-[250px] whitespace-normal">
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger>
                          <span className="truncate max-w-[150px] block cursor-default">
                  ...
                  <TableCell className="border-b border-r pl-[54px] pe-[41px] py-[22px] text-nowrap text-sm leading-4">
                    {format(seller.createdDate, "PPPp")}
                  </TableCell>
                  {/* <TableCell className="border-b border-r pl-[54px] pe-[41px] py-[22px] text-nowrap text-sm leading-4">
                    {format(seller.createdDate, "p")}
                  </TableCell> */}
                  ...
                  <TableCell className="border-b border-r pl-[54px] pe-[41px] py-[22px] text-nowrap text-sm leading-4">
                    <div
                      className={cn(
                        ...
                        seller.status === "INCOMPLETE" &&
                          "bg-[#FF4D011A] text-[#FF4D01]",
                      )}
                    >
                      {convertPascalCase(seller.status)}
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="border-b pl-[54px] pe-[41px] py-[22px]">
                    <div className="flex justify-center">
                      <DropdownMenu
                        open={openDropdown === seller.id}
                        onOpenChange={(open) =>
                          setOpenDropdown(open ? seller.id : null)
                        }
                      >
                        ...
                        >
                          ...
                          >
                            <Link
                              ...
                              href={ROUTES.VIEW_SELLER_DETAILS(seller.id)}
                            >
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="16"
                          ...
                          {seller.status !== "INCOMPLETE" ? (
                            <>
                              ...
                              {seller.status !== "APPROVED" && (
                                <DropdownMenuItem
                                  className="p-0 cursor-pointer hover:bg-transparent hover:text-green-500 group text-[14px] text-[#122D4FCC] "
                                  // onClick={() =>
                                  //   sellerActionMutate({
                                  //     id: seller.id,
                                  //     type: "APPROVED",
                              ...
                              >
                                <path
                                  ...
                                  stroke-linecap="round"
                                  stroke-linejoin="round"
                                  className="group-hover:stroke-green-500 stroke-[#122D4F]"
                                />
                                <path
                                  d="M7 10L10.7733 6"
                                  stroke="white"
                              ...
                              {seller.status !== "REJECTED" && (
                                <DropdownMenuItem
                                  className="w-full flex justify-start items-center gap-2 p-0 text-[14px] text-[#122D4FCC] hover:text-red-500 cursor-pointer hover:bg-transparent  group"
                                  // onClick={() =>
                                  //   sellerActionMutate({
                                  //     id: seller.id,
                                  //     type: "REJECTED",
                              ...
                              >
                                <path
                                  ...
                                  stroke-linecap="round"
                                  stroke-linejoin="round"
                                  className="group-hover:stroke-red-500 stroke-[#122D4F]"
                                />
                                <path
      ...
      <Dialog open={succOpen} onOpenChange={closeModel}>
      ...
      <Dialog open={succOpen1} onOpenChange={closeModel1}>
